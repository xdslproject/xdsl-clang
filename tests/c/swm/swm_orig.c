...
static float u1[N_LEN * M_LEN], u2[N_LEN * M_LEN], u3[N_LEN * M_LEN];
static float v1[N_LEN * M_LEN], v2[N_LEN * M_LEN], v3[N_LEN * M_LEN];
static float p1[N_LEN * M_LEN], p2[N_LEN * M_LEN], p3[N_LEN * M_LEN];

static float cu[N_LEN * M_LEN];
static float cv[N_LEN * M_LEN];
static float z[N_LEN * M_LEN];
static float h[N_LEN * M_LEN];
static float psi[N_LEN * M_LEN];
...
static void dswap(float **a, float **b) {
  float *t = *a;
  *a = *b;
  *b = t;
}
...
int main(void) {
  ...
  float a_const = 1.0e6f, alpha = 0.001f;
  float el = (float)N * dx;
  float pi = atan2f(0.0f, -1.0f);
  float tpi = pi + pi;
  float di = tpi / (float)M;
  float dj = tpi / (float)N;
  float pcf = pi * pi * a_const * a_const / (el * el);
  ...
  for (int j = 0; j < N_LEN; j++) {
    for (int i = 0; i < M_LEN; i++) {
      psi[j * M_LEN + i] = a_const * sinf((i + 0.5f) * di) * sinf((j + 0.5f) * dj);
      p[j * M_LEN + i] =
