...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-D", "--define-macro", action="append", dest="defines", default=[],
        help="Preprocessor define (passed to clang)",
    )
    parser.add_argument(
        "-I", "--include-directory", action="append", dest="includes", default=[],
        help="Include directory (passed to clang)",
    )
    parser.add_argument(
        "--linkobj", action="append", dest="linkobjs", default=[],
        help="Additional object/bitcode files to link against",
    )
    parser.add_argument(
        "-e", "--extra-pass", action="append", dest="extra_passes", default=[],
...
def _enabled_stages(out_type: OutputType, override: str | None) -> dict[str, bool]:
    ...
    stages = {
        "clang": True,
        "cir":   True,
        "post":  out_type in (OutputType.EXECUTABLE, OutputType.OBJECT, OutputType.BITCODE),
        "mlir":  out_type in (OutputType.EXECUTABLE, OutputType.OBJECT, OutputType.BITCODE),
...
def main() -> int:
    ...
    if stages["clang"]:
        _run(
            [args.clang, "-fclangir", "-emit-cir", "-O0",
             "-Xclang", "-no-enable-noundef-analysis",
             *define_args, *include_args, src, "-o", cir_path],
            verbose=args.verbose, stage="clang",
        )
    ...
    if stages["cir"]:
        ...
        _run(
            ...
            verbose=args.verbose, stage="cir",
        )

    if stages["post"]:
        # Currently a no-op fixup; kept as a seam.
        _shell(f"cp {shlex.quote(res_path)} {shlex.quote(post_path)}",
               verbose=args.verbose, stage="post")

    if stages["mlir"]:
        bc_target = out if out_type == OutputType.BITCODE else bc_path
        _shell(
            f"{shlex.quote(args.mlir_opt)} --pass-pipeline={shlex.quote(MLIR_PIPELINE)} "
            f"{shlex.quote(post_path)} | "
            f"{shlex.quote(args.mlir_translate)} --mlir-to-llvmir -o {shlex.quote(bc_target)}",
            verbose=args.verbose, stage="mlir",
        )

    if stages["obj"]:
        _run(
            [args.clang, "-O3", "-c", bc_path, "-o", out],
            verbose=args.verbose, stage="obj",
        )

    if stages["exe"]:
        _run(
            [args.clang, "-O3", bc_path, *args.linkobjs, "-lm", "-o", out],
            verbose=args.verbose, stage="exe",
        )
